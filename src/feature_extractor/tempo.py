...
class CyclicTempogram:
    ...
    def extract(self, audio):
        ...
        for m in range(len(onset_env)):
            ...
            r_tau = np.correlate(o_frame, o_frame, mode="full")[self.win_length - 1:]
            # fourier Transform
    ...
    def plot(self, feature_matrix, ax):
        bpm_freqs = np.arange(feature_matrix.shape[0]) * (self.sr * 60 / (self.hop_length * self.win_length))
        times = librosa.frames_to_time(np.arange(feature_matrix.shape[1]), sr=self.sr, hop_length=self.hop_length)
        feature_matrix = feature_matrix[1:, :]
        bpm_freqs = bpm_freqs[1:]
        ax.pcolormesh(times, bpm_freqs, feature_matrix, cmap="coolwarm", shading="nearest")
